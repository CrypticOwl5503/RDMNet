...
def fit_one_epoch(model_train, model, yolo_loss, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, save_period):
    ...
    with tqdm(total=epoch_step, desc=f'Epoch {epoch + 1}/{Epoch}', postfix=dict, mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen):
            ...
            if len(batch) == 5:
                # With context pairs
                images, targets, clearimgs, degrad_contexts, clean_contexts = batch
                context_images = (degrad_contexts, clean_contexts)
            else:
                # Without context pairs (backward compatible)
            ...
            with torch.no_grad():
                ...
                if cuda:
                    ...
                    clearimgs = clearimgs.cuda()
                    if context_images is not None:
                        context_images = (context_images[0].cuda(), context_images[1].cuda())
                
                posimgs = augment(images)
                hazy_and_clear = torch.cat([images, posimgs], dim=0)
                if cuda:
                    hazy_and_clear = hazy_and_clear.cuda()

            optimizer.zero_grad()

            # Pass context_images to model if available
            if context_images is not None:
                detected, restored, logits, labels = model_train(hazy_and_clear, context_images=context_images)
            else:
    ...
    with tqdm(total=epoch_step_val, desc=f'Epoch {epoch + 1}/{Epoch}', postfix=dict, mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen_val):
            ...
            if len(batch) == 5:
                # With context pairs
                images, targets, _, degrad_contexts, clean_contexts = batch
                context_images = (degrad_contexts, clean_contexts)
            else:
                # Without context pairs (backward compatible)
            ...
            with torch.no_grad():
                ...
                # Pass context_images to model if available
                if context_images is not None:
                    detected, restored = model_train(images, context_images=context_images)
                else:
